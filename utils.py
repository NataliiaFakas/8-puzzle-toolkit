...
def getManhattanDistance(state):
    ...
    for i in range(1,9):
        ...
        goal_pos_col = goal_pos % 3

        state_pos = state.index(str(i))
        state_pos_row = int(state_pos / 3)
        state_pos_col = state_pos % 3

        h += (abs(goal_pos_row - state_pos_row) + 
              abs(goal_pos_col - state_pos_col))
    return h
